...
export default function ProveFairness({ random, hash }) {
  ...
  return (
    <div className="border rounded my-2 p-18 bg-white text-gray-900">
      ...
      {!hashedProof && !playerProof && (
        <div className="border p-8 my-4 w-full">
          ...
          <input
            className="border rounded-sm border-gray-700 w-full p-3"
            type="text"
            value={ethAddress}
          ...
          <div className="my-2 w-full">
            {ethAddress.length === 42 && (
              <button
                onClick={prove}
                className="border w-full rounded-sm text-white bg-gray-500 px-10 py-2">
                Prove
              </button>
            )}
          </div>
